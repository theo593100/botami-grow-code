...
const valueProps = [
  { icon: Receipt, title: "Votre modèle de facturation, pas un standard", desc: "Récurrent, ponctuel, à l'avancement, au forfait, au temps passé — votre activité a son modèle. Un logiciel de facturation générique vous force à bidouiller. Le vôtre gère votre cas natif." },
  { icon: ShieldCheck, title: "Conforme facturation électronique 2026", desc: "La réforme française impose la facturation électronique pour toutes les entreprises. On construit directement conforme : format Factur-X, connexion PDP, archivage probant. Pas besoin de changer d'outil en 2026." },
  { icon: Link2, title: "Connecté à votre compta", desc: "Export FEC, API Pennylane, Sage, Cegid, EBP — votre expert-comptable récupère les écritures sans ressaisie. Vous gardez votre cabinet, vous gagnez le temps perdu en allers-retours." },
  { icon: BadgeDollarSign, title: "Prix forfaitaire, pas d'abonnement", desc: "Pas 25€/mois × 12 × années qui passent. Un forfait, une fois. Le code est à vous. Si vous facturez 50 ou 5 000 documents par an, le prix est le même — zéro." },
];
...
const comparatifRows = [
  { critere: "Modèle de prix", concurrent: "Abonnement 15-50€/mois + options (multi-devises, récurrent, API)", botami: "Forfait 5 000-15 000€, une seule fois" },
  { critere: "Propriété du code", concurrent: "Aucune", botami: "Oui, code source remis" },
  { critere: "Adaptation à votre modèle", concurrent: "Limitée aux options prévues", botami: "Sur mesure (récurrent, avancement, multi-entités, règles fiscales spécifiques)" },
  { critere: "Facturation électronique 2026", concurrent: "Incluse, dépendante de leur roadmap", botami: "Intégrée nativement dès la conception" },
  { critere: "Dépendance au fournisseur", concurrent: "Totale", botami: "Zéro — données et hébergement chez vous" },
  { critere: "Coût sur 3 ans", concurrent: "900 à 3 000€ (logiciel seul, hors options)", botami: "5 000-15 000€ (payé une fois)" },
];
...
const faqs = [
  { q: "On a déjà Sellsy / Pennylane / Indy, ça vaut le coup de refaire ?", a: "Honnêtement, pour la plupart des PME avec un modèle de facturation simple, non. Ces outils font très bien le job pour 20-40€/mois. On intervient quand votre modèle devient spécifique (facturation à l'avancement complexe, multi-entités, règles fiscales particulières, récurrent avec exceptions) ou quand vous accumulez les options payantes au point que le sur mesure devient plus rentable." },
  { q: "Comment vous gérez la facturation électronique 2026 ?", a: "Conforme dès la conception : format Factur-X, connexion à une PDP (plateforme de dématérialisation partenaire) au choix, archivage probant. On suit la roadmap officielle de la DGFiP. Votre logiciel est prêt avant l'obligation." },
  { q: "Et pour les relances impayées ?", a: "Scénarios de relance automatiques selon le retard (J+7, J+15, J+30 — configurable) : email, courrier, tâche commerciale. Génération automatique des pré-lettres de mise en demeure. Tableau de bord des impayés en temps réel." },
  { q: "Ça se connecte à mon expert-comptable ?", a: "Oui. Export FEC conforme, connexion API avec les outils courants (Pennylane, Sage, Cegid, EBP). Votre cabinet récupère les écritures proprement, sans ressaisie. Vous continuez avec votre comptable actuel." },
  { q: "Je peux facturer en plusieurs devises ou pour plusieurs entités ?", a: "Oui, si c'est au cahier des charges. Multi-devises avec taux de change, multi-entités (plusieurs sociétés dans le même outil), TVA intracommunautaire, autoliquidation — toutes les règles fiscales spécifiques sont prises en compte dès la conception." },
];
  ...
  return (
    <div className={body} style={{ backgroundColor: C.bg, color: C.text }}>
      ...
      <section className="px-4 md:px-8 py-16 md:py-24" style={{ backgroundColor: C.card }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            <h2 className={`${heading} text-2xl md:text-4xl mb-4 text-center`}>Logiciel de facturation standard vs Botami Software</h2>
            <p className="text-lg text-center mb-14 max-w-2xl mx-auto" style={{ color: C.textSec }}>Pourquoi un SaaS de facturation générique atteint vite ses limites.</p>
          </Reveal>
          <Reveal delay={100}>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr style={{ backgroundColor: C.bg }}>
                    ...
                    <th className="px-6 py-4 text-sm font-semibold" style={{ color: C.text }}>SaaS facturation standard (Sellsy, Pennylane…)</th>
              ...
              <Reveal key={i} delay={i * 100}>
                <div className="rounded-xl border p-5" style={{ backgroundColor: C.card, borderColor: "#E5E7EB" }}>
                  ...
                  <div className="space-y-3">
                    <div><p className="text-xs font-medium mb-1" style={{ color: C.textSec }}>SaaS standard</p><p className="text-sm" style={{ color: C.textSec }}>{r.concurrent}</p></div>
                    <div className="pt-2 border-t" style={{ borderColor: "#E5E7EB" }}><p className="text-xs font-medium mb-1" style={{ color: C.amber }}>Botami Software</p><p className="text-sm font-semibold" style={{ color: C.amber }}>{r.botami}</p></div>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          <Reveal delay={200}>
            <p className="text-sm mt-8 text-center max-w-3xl mx-auto" style={{ color: C.textSec }}>
              Un SaaS standard reste moins cher si vos besoins de facturation sont simples et tiennent dans les options prévues. Le sur mesure devient pertinent quand votre modèle est spécifique, quand vous accumulez les options payantes, ou quand vous voulez possession totale de vos données et de votre logique de facturation.
            </p>
          </Reveal>
        </div>
      </section>
            ...
            <Accordion type="single" collapsible defaultValue="item-0">
              {faqs.map((f, i) => (
                <AccordionItem key={i} value={`item-${i}`} className="border-b" style={{ borderColor: "#E5E7EB" }}>
                  <AccordionTrigger className="text-left font-heading font-semibold text-base py-5 hover:no-underline" style={{ color: C.text }}>{f.q}</AccordionTrigger>
      ...
      {/* Réservation */}
      <LPFormCalendly route="/lp/google/logiciel-facturation" title="Réservez votre appel découverte gratuit" subtitle="15-20 minutes pour évaluer votre besoin. On vous dit honnêtement si un logiciel de facturation sur mesure est pertinent pour votre cas, ou pas." buttonLabel="Je réserve mon appel découverte gratuit" />

      {/* Dernier CTA */}
      <section className="px-4 md:px-8 py-16 md:py-20 text-center" style={{ backgroundColor: C.amberLight }}>
        <div className="max-w-2xl mx-auto">
          <Reveal><p className="text-lg md:text-xl font-medium mb-8" style={{ color: C.text }}>🇫🇷 Entreprise française. Code source remis à chaque projet. Prix forfaitaire. Conforme facturation électronique 2026.</p></Reveal>
...
export default LandingGoogleLogicielFacturation;
