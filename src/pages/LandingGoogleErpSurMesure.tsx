...
const valueProps = [
  { icon: Settings, title: "Construit pour votre métier, pas l'inverse", desc: "Un ERP standard vous force à rentrer dans ses cases. Changement de nomenclature, processus à adapter, équipes à former sur un outil qui ne parle pas leur langage. On inverse le rapport de force : l'outil s'adapte à vous." },
  { icon: Package, title: "Modules utiles, pas modules gadgets", desc: "On ne vend pas de suite à 47 modules dont vous n'utiliserez que 6. Cahier des charges = périmètre exact. Vous payez ce qui sert, rien d'autre. Ajouts possibles plus tard si le besoin apparaît." },
  { icon: Rocket, title: "Implémentation en semaines, pas en années", desc: "Un ERP standard = 6 à 18 mois d'implémentation, consultants externes, comité de pilotage, dérives de budget. Chez nous : 4 à 8 semaines, vous êtes en production. Pas parce qu'on bâcle — parce qu'on cadre le scope." },
  { icon: CreditCard, title: "Prix forfaitaire, pas licence perpétuelle", desc: "Pas de licence par utilisateur, pas d'abonnement au module, pas de facture à chaque évolution. Un forfait, une fois. Le code vous appartient. Un autre dev peut reprendre." },
];

const steps = [
  { num: "01", icon: FileText, title: "Cahier des charges", time: "1-2 semaines", desc: "On cartographie vos flux : ventes, achats, stocks, production si applicable, facturation, reporting. On identifie les règles de gestion, les circuits de validation, les interfaces avec la compta et la paie." },
  { num: "02", icon: Layers, title: "Maquette", time: "1-2 semaines", desc: "Prototype cliquable de tous les modules du périmètre : saisie commande, gestion stock, bon de livraison, facture, tableaux de bord. Vous validez avant le code." },
  { num: "03", icon: Hammer, title: "Développement", time: "4-8 semaines", desc: "Construction itérative sur un environnement de staging. Vous testez chaque semaine avec vos vraies données. Ajustements au fil de l'eau." },
  { num: "04", icon: PackageCheck, title: "Livraison + formation", time: "1 semaine", desc: "Mise en production. Import de vos données existantes (clients, articles, fournisseurs). Formation des équipes. Remise du code source, des accès et de la documentation." },
];

const comparatifRows = [
  { critere: "Modèle de prix", standard: "Licence + implémentation + abonnement par utilisateur", botami: "Forfait 5 000-15 000€, une seule fois" },
  { critere: "Propriété du code", standard: "Aucune (licence ou open-source avec tuteur)", botami: "Oui, code source remis" },
  { critere: "Délai d'implémentation", standard: "6 à 18 mois", botami: "4 à 8 semaines" },
  { critere: "Adaptation métier", standard: "Via modules payants et développements spécifiques facturés à part", botami: "Sur mesure dès la conception" },
  { critere: "Dépendance au fournisseur", standard: "Forte (intégrateur, consultants, support)", botami: "Zéro — vous partez avec tout" },
  { critere: "Coût sur 3 ans", standard: "30 000 à 120 000€ selon taille", botami: "5 000-15 000€ (payé une fois)" },
];

const faqs = [
  { q: "Un ERP sur mesure à 5-15k€, vraiment ?", a: "Oui, si on cadre le scope. Un ERP qui gère 4 modules utiles pour une PME, c'est faisable en 4-8 semaines. Un ERP qui couvre 47 modules comme SAP, non. Notre cible : les PME qui ont besoin d'un outil métier efficace, pas d'une usine à gaz." },
  { q: "Et pour la compta, ça se connecte à notre expert-comptable ?", a: "Oui. Export FEC, API Pennylane, Sage, Cegid, ou connexion directe selon ce que votre cabinet utilise. On ne refait pas la compta — on envoie les écritures proprement." },
  { q: "On peut faire évoluer l'ERP dans 2 ans quand on grandira ?", a: "Oui. Le code vous appartient. Soit on ajoute des modules au cas par cas, soit vous reprenez en interne, soit un autre dev reprend. Pas de verrou." },
  { q: "Comment on migre depuis notre ERP actuel ?", a: "Inclus dans la semaine de livraison. Export de vos données existantes (clients, produits, fournisseurs, commandes en cours) puis import dans le nouvel outil. Historique gardé si exportable." },
  { q: "On a déjà Odoo / SAP / Cegid, ça vaut le coup de refaire ?", a: "Si votre ERP actuel fait le job, gardez-le. On intervient quand les coûts d'abonnement explosent, quand les évolutions prennent 6 mois, ou quand vos équipes contournent l'outil via Excel parce qu'il ne correspond plus au métier. Bilan honnête en appel découverte." },
];
...
const ProcessTimeline = () => {
  ...
  useEffect(() => {
    document.title = "ERP sur mesure pour PME | Botami Software";
    const meta = document.querySelector('meta[name="description"]');
    if (meta) meta.setAttribute("content", "ERP sur mesure conçu pour votre métier. Forfait 5 000–15 000€, livré en 4 à 8 semaines, code source à vous.");
  }, []);
...
const LandingGoogleErpSurMesure = () => {
  return (
    <div className={body} style={{ backgroundColor: C.bg, color: C.text }}>
      ...
      <section className="px-4 md:px-8 pt-16 pb-20 md:pt-24 md:pb-28">
        <div className="max-w-3xl mx-auto text-center">
          <Reveal><p className="text-sm font-semibold uppercase tracking-widest mb-5" style={{ color: C.amber }}>ERP sur mesure</p></Reveal>
          <Reveal delay={100}><h1 className={`${heading} text-3xl md:text-5xl lg:text-[3.5rem] leading-tight mb-6`}>ERP sur mesure. Le vôtre, pas celui d'SAP, d'Odoo ou de votre voisin.</h1></Reveal>
          <Reveal delay={200}><p className="text-lg md:text-xl leading-relaxed mb-10" style={{ color: C.textSec }}>Ventes, achats, stocks, production, facturation — on construit l'ERP autour de votre manière de travailler. Pas d'implémentation de 18 mois, pas de consultants qui vous expliquent votre métier. Forfait 5 000 à 15 000€, 4 à 8 semaines, code source à vous.</p></Reveal>
      ...
      <section className="px-4 md:px-8 py-16 md:py-24">
        <div className="max-w-5xl mx-auto">
          ...
          <Reveal delay={100}><div className="flex flex-wrap items-center justify-center gap-8 mb-12">{clientLogos.map((l, i) => (<img key={i} src={l.src} alt={l.alt} className="h-10 w-auto object-contain opacity-70 hover:opacity-100 transition-opacity grayscale hover:grayscale-0" />))}</div></Reveal>
      ...
      <section className="px-4 md:px-8 py-16 md:py-24" style={{ backgroundColor: C.card }}>
        <div className="max-w-4xl mx-auto">
          <Reveal><h2 className={`${heading} text-2xl md:text-4xl mb-4 text-center`}>ERP standard vs Botami Software</h2></Reveal>
          <Reveal delay={100}><p className="text-lg text-center mb-12" style={{ color: C.textSec }}>Pourquoi un ERP du marché finit souvent en chantier sans fin.</p></Reveal>
          <Reveal delay={200}>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead><tr>
                  ...
                  <th className={`${heading} text-sm uppercase tracking-wide py-4 px-4 border-b-2`} style={{ borderColor: "#E5E7EB", color: C.textSec }}>ERP standard</th>
      ...
      <LPFormCalendly route="/lp/google/erp-sur-mesure" title="Réservez votre appel découverte gratuit" subtitle="15-20 minutes pour évaluer votre besoin. On vous dit honnêtement si un ERP sur mesure est pertinent pour votre cas, ou pas." />

      <section className="px-4 md:px-8 py-16 md:py-20 text-center" style={{ backgroundColor: C.amberLight }}>
        <div className="max-w-2xl mx-auto">
          <Reveal><p className="text-lg md:text-xl font-medium mb-8" style={{ color: C.text }}>🇫🇷 Entreprise française. Code source remis à chaque projet. Prix forfaitaire. Votre ERP, pas celui du voisin.</p></Reveal>
...
export default LandingGoogleErpSurMesure;
