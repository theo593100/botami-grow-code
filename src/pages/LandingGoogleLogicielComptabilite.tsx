...
const valueProps = [
  { icon: Workflow, title: "On ne refait pas la compta de votre cabinet", desc: "Sage, Cegid, Quadratus, Pennylane — votre expert-comptable a son outil, il est très bien. On construit la brique qui alimente son logiciel sans ressaisie : pré-compta, rapprochement bancaire automatique, export FEC propre." },
  { icon: BarChart3, title: "Reporting financier adapté à votre business", desc: "Un bilan N-1 ne vous pilote pas au quotidien. On construit les tableaux de bord que vous voulez vraiment : marge par client, par projet, par ligne de produit, DSO, trésorerie prévisionnelle. En temps réel, pas 3 mois en retard." },
  { icon: ScanLine, title: "Saisie automatique, ressaisie zéro", desc: "Factures fournisseurs captées par email ou scan, OCR, affectation comptable automatique, validation humaine si besoin, envoi au cabinet. Vos équipes arrêtent de recopier des factures." },
  { icon: BadgeDollarSign, title: "Prix forfaitaire, outil à vous", desc: "Pas d'abonnement qui s'empile en plus de votre cabinet. Un forfait, une fois. Code source à vous. Si votre expert-comptable change demain, votre outil continue de fonctionner." },
];

const clientLogos = [
  { src: logoBlackfox, alt: "Blackfox" }, { src: logoDesMurs, alt: "Des Murs à Paris" }, { src: logoErgosante, alt: "Ergo Santé" }, { src: logoJD, alt: "JD Sports" }, { src: logoKijiji, alt: "Kijiji" }, { src: logoLibralerte, alt: "Libr'Alerte" }, { src: logoProvence, alt: "Mademoiselle Provence" }, { src: logoNotaires, alt: "Notaires de France" }, { src: logoPoupina, alt: "Poupina" }, { src: logoProarti, alt: "Proarti" }, { src: logoSkills, alt: "Skills Communication" }, { src: logoSkinCafeine, alt: "Skin Cafeine" }, { src: logoStAubin, alt: "St-Aubin Avocats" },
];

const comparatifRows = [
  { critere: "Usage principal", concurrent: "Tenue de comptabilité par un expert", botami: "Outil pré-compta, automatisations, reporting financier" },
  { critere: "Modèle de prix", concurrent: "Abonnement 20-100€/mois + modules", botami: "Forfait 5 000-15 000€, une seule fois" },
  { critere: "Positionnement", concurrent: "Remplace ou complète le travail comptable", botami: "Vient en amont, prépare et alimente le logiciel du cabinet" },
  { critere: "Personnalisation du reporting", concurrent: "Limitée aux rapports pré-formatés", botami: "Sur mesure, tableaux de bord construits pour votre business" },
  { critere: "Propriété du code", concurrent: "Aucune", botami: "Oui, code source remis" },
  { critere: "Complémentarité avec l'expert-comptable", concurrent: "Oui, mais l'expert utilise son propre outil", botami: "Pensé pour s'interfacer proprement avec le cabinet" },
];
...
const faqs = [
  { q: "Vous remplacez mon expert-comptable ?", a: "Non, jamais. On construit un outil qui lui facilite la vie. Votre expert-comptable reste, son logiciel reste. On automatise ce qui arrive dans son outil pour qu'il passe moins de temps sur la saisie et plus sur le conseil. C'est généralement lui le premier content." },
  { q: "Vous vous connectez à quels logiciels comptables ?", a: "Tous les courants : Sage, Cegid, Quadratus, Pennylane, Tiime, EBP, Indy — soit via API, soit via export FEC. On s'adapte à ce que votre cabinet utilise, pas l'inverse." },
  { q: "Vous gérez la TVA ?", a: "Déclaration de TVA, non — c'est le rôle de votre expert-comptable. Préparation automatique des éléments (collecte factures, calcul TVA collectée/déductible, mise en forme), oui. Votre cabinet reçoit tout propre et valide." },
  { q: "Et le rapprochement bancaire ?", a: "Automatisé via connexion aux agrégateurs bancaires (Budget Insight, Bridge, ou équivalent). Les écritures sont rapprochées automatiquement, validation humaine uniquement sur les cas ambigus. Gain de temps typique : 2-5 jours par mois selon le volume." },
  { q: "Combien de temps pour être opérationnel ?", a: "4 à 8 semaines de la signature à la mise en production, incluant la formation et la connexion au logiciel de votre expert-comptable. Le cabinet est impliqué dès le cahier des charges pour s'assurer que l'interface fonctionne." },
];
    ...
    <div className={body} style={{ backgroundColor: C.bg, color: C.text }}>
      ...
      <section className="px-4 md:px-8 pt-16 pb-20 md:pt-24 md:pb-28">
        <div className="max-w-3xl mx-auto text-center">
          <Reveal><p className="text-sm font-semibold uppercase tracking-widest mb-5" style={{ color: C.amber }}>Logiciel comptabilité sur mesure</p></Reveal>
          <Reveal delay={100}><h1 className={`${heading} text-3xl md:text-5xl lg:text-[3.5rem] leading-tight mb-6`}>Logiciel comptabilité sur mesure. Pas pour remplacer votre expert-comptable, pour lui faciliter la vie.</h1></Reveal>
          <Reveal delay={200}><p className="text-lg md:text-xl leading-relaxed mb-10" style={{ color: C.textSec }}>Pré-compta, automatisation des saisies, reporting financier sur mesure, interfaces avec Sage, Cegid, Pennylane, Tiime. Vous gardez votre cabinet, vous gagnez le temps perdu. Forfait 5&nbsp;000 à 15&nbsp;000€, 4 à 8 semaines, code source à vous.</p></Reveal>
      ...
      <section className="px-4 md:px-8 py-16 md:py-24">
        <div className="max-w-5xl mx-auto">
          <Reveal><p className={`${heading} text-lg md:text-xl text-center mb-10`}>Notre équipe accompagne des entreprises en acquisition digitale depuis des années</p></Reveal>
          ...
          <Reveal delay={200}>
            <div className="relative max-w-2xl mx-auto px-6 py-8">
              ...
              <p className="text-center text-lg md:text-xl leading-relaxed italic" style={{ color: C.textSec }}>
                On les a vues se battre avec des{" "}<span className="font-semibold" style={{ color: C.amber }}>SaaS trop chers</span>, des{" "}<span className="font-semibold" style={{ color: C.amber }}>Excel qui craquent</span>, des outils qui ne collent pas à leur métier.<br className="hidden md:block" />{" "}<span className="font-bold" style={{ color: C.text }}>Botami Software</span> est né de ce constat&nbsp;:{" "}<span className="font-semibold" style={{ color: C.amber }}>quand les outils du marché ne suffisent plus</span>, on construit celui qu'il vous faut.
              </p>
            </div>
          </Reveal>
        </div>
      ...
      <section className="px-4 md:px-8 py-16 md:py-24" style={{ backgroundColor: C.card }}>
        <div className="max-w-5xl mx-auto">
          <Reveal>
            <h2 className={`${heading} text-2xl md:text-4xl mb-4 text-center`}>Outils compta standard vs Botami Software</h2>
            <p className="text-lg text-center mb-14 max-w-2xl mx-auto" style={{ color: C.textSec }}>Pourquoi on ne se positionne pas sur le même terrain qu'un logiciel comptable.</p>
          </Reveal>
          <Reveal delay={100}>
            <div className="hidden md:block rounded-2xl border overflow-hidden" style={{ borderColor: "#E5E7EB" }}>
              <table className="w-full text-left">
                <thead><tr style={{ backgroundColor: C.bg }}><th className="px-6 py-4 text-sm font-semibold" style={{ color: C.text }}>Critère</th><th className="px-6 py-4 text-sm font-semibold" style={{ color: C.text }}>Logiciel comptable standard (Sage, Cegid…)</th><th className="px-6 py-4 text-sm font-semibold" style={{ color: C.amber }}>Botami Software</th></tr></thead>
                <tbody>{comparatifRows.map((r, i) => (<tr key={i} className="border-t" style={{ borderColor: "#E5E7EB", backgroundColor: i % 2 === 0 ? C.card : C.bg }}><td className="px-6 py-4 text-sm font-medium" style={{ color: C.text }}>{r.critere}</td><td className="px-6 py-4 text-sm" style={{ color: C.textSec }}>{r.concurrent}</td><td className="px-6 py-4 text-sm font-semibold" style={{ color: C.amber }}>{r.botami}</td></tr>))}</tbody>
              </table>
            </div>
          </Reveal>
          <div className="md:hidden flex flex-col gap-4">
            {comparatifRows.map((r, i) => (<Reveal key={i} delay={i * 100}><div className="rounded-xl border p-5" style={{ backgroundColor: C.card, borderColor: "#E5E7EB" }}><h4 className="text-sm font-semibold mb-3" style={{ color: C.text }}>{r.critere}</h4><div className="space-y-3"><div><p className="text-xs font-medium mb-1" style={{ color: C.textSec }}>Logiciel standard</p><p className="text-sm" style={{ color: C.textSec }}>{r.concurrent}</p></div><div className="pt-2 border-t" style={{ borderColor: "#E5E7EB" }}><p className="text-xs font-medium mb-1" style={{ color: C.amber }}>Botami Software</p><p className="text-sm font-semibold" style={{ color: C.amber }}>{r.botami}</p></div></div></div></Reveal>))}
          </div>
          <Reveal delay={200}><p className="text-sm mt-8 text-center max-w-3xl mx-auto" style={{ color: C.textSec }}>Si vous cherchez à remplacer le logiciel de votre expert-comptable, on n'est pas la bonne réponse. Si vous cherchez à automatiser et fiabiliser ce qui entre dans son logiciel, on l'est.</p></Reveal>
        </div>
      </section>
      ...
      <section className="px-4 md:px-8 py-16 md:py-24" style={{ backgroundColor: C.card }}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-10 md:gap-14 max-w-3xl mx-auto mb-14 mt-14">
            {team.map((t, i) => (<Reveal key={t.name} delay={i * 200}><div className="flex flex-col items-center text-center"><div className="relative w-full max-w-xs aspect-[3/4] mb-6 overflow-hidden rounded-2xl group"><img src={t.photo} alt={t.name} className="absolute inset-0 w-full h-full object-cover grayscale transition-transform duration-700 ease-out group-hover:scale-105" style={{ objectPosition: t.objectPosition }} /><div className="absolute bottom-0 left-0 right-0 h-1/4 bg-gradient-to-t from-black/30 to-transparent pointer-events-none" /></div><h3 className={`${heading} text-xl mb-1`}>{t.name}</h3><p className="text-sm font-semibold mb-3" style={{ color: C.amber }}>{t.role}</p><p className="text-sm leading-relaxed max-w-sm" style={{ color: C.textSec }}>{t.desc}</p></div></Reveal>))}
          </div>
          <Reveal>
            <div className="rounded-2xl p-6 md:p-8 border" style={{ backgroundColor: C.bg, borderColor: "#E5E7EB" }}>
              ...
              <div className="grid sm:grid-cols-2 gap-4">{guarantees.map((g, i) => (<div key={i} className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: C.success }} /><span className="leading-relaxed text-sm">{g}</span></div>))}</div>
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <Reveal delay={200}>
            <Accordion type="single" collapsible defaultValue="item-0">
              {faqs.map((f, i) => (<AccordionItem key={i} value={`item-${i}`} className="border-b" style={{ borderColor: "#E5E7EB" }}><AccordionTrigger className="text-left font-heading font-semibold text-base py-5 hover:no-underline" style={{ color: C.text }}>{f.q}</AccordionTrigger><AccordionContent className="leading-relaxed pb-5" style={{ color: C.textSec }}>{f.a}</AccordionContent></AccordionItem>))}
            </Accordion>
          </Reveal>
        </div>
      </section>

      <LPFormCalendly route="/lp/google/logiciel-comptabilite" title="Réservez votre appel découverte gratuit" subtitle="15-20 minutes pour évaluer votre besoin. On vous dit honnêtement si un outil sur mesure est pertinent pour votre cas, ou pas." buttonLabel="Je réserve mon appel découverte gratuit" />

      <section className="px-4 md:px-8 py-16 md:py-20 text-center" style={{ backgroundColor: C.amberLight }}>
        <div className="max-w-2xl mx-auto">
          <Reveal><p className="text-lg md:text-xl font-medium mb-8" style={{ color: C.text }}>🇫🇷 Entreprise française. Code source remis à chaque projet. Prix forfaitaire. Votre expert-comptable reste, votre process s'automatise.</p></Reveal>
...
export default LandingGoogleLogicielComptabilite;
